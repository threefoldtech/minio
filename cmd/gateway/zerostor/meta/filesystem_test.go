...
func TestScan(t *testing.T) {
	...
	files := []string{
		...
		"media/series/firends/s1.mp4",
		"media/series/firends/s2.mp4",
		"media/music/sound of silence.mp3",
	}

	mgr := newFilesystemMetaManager(dir, "")
	bucket := "bucket"

	for _, file := range files {

		mgr.PutObject(&metatypes.Metadata{}, bucket, file)
		require.NoError(err)
	}
	...
	// results = cmd.ListObjectsV2Info{}
	// _, err = mgr.scan(ctx, "bucket", "", "documents", false, 500, &results)
	// require.NoError(err)

	// require.Len(results.Objects, 2) // files plus directories
